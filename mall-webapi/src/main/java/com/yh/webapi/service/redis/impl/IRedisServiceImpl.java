package com.yh.webapi.service.redis.impl;

import com.yh.webapi.service.redis.IRedisService;
...
@Slf4j
@Service
public class IRedisServiceImpl implements IRedisService<String, Object> {
    ...
    @Override
    public Object getMapField(String key, String filed) {
        return redisTemplate.boundHashOps(key).get(filed);
    }
    ...
    private static String buildSignKey(String userKey, LocalDate date) {
        return String.format("user:sign:%s:%s", userKey, formatDate(date));
    }
    ...
    @Override
    public Boolean doSign(String userKey, LocalDate date) {
        //偏移量（下标）
        int offset = date.getDayOfMonth() - 1;
        //user:sign:1:202012 2 1 (2020-12-3 签到)
        return redisTemplate.opsForValue().setBit(buildSignKey(userKey, date), offset, true);
    }

    @Override
    public Boolean checkSign(String userKey, LocalDate date) {
        int offset = date.getDayOfMonth() - 1;
        return redisTemplate.opsForValue().getBit(buildSignKey(userKey, date), offset);
    }


    @Override
    public Long getSignCount(String userKey, LocalDate date) {
        return (Long) redisTemplate.execute((RedisCallback<Long>) conn -> conn.bitCount(buildSignKey(userKey, date).getBytes()));
    }


    private  List<Long> bitfield(String buildSignKey,int limit,int offset){
        return (List<Long>)redisTemplate.execute((RedisCallback<List<Long>>) conn->conn.bitField(buildSignKey.getBytes(),
                BitFieldSubCommands.create().get(BitFieldSubCommands.BitFieldType.unsigned(limit)).valueAt(offset)));
    }

    @Override
    public Long getContinuousSignCount(String userKey, LocalDate date) {
        int signCount = 0;
        List<Long> list = bitfield(buildSignKey(userKey,date),date.getDayOfMonth(),0);
        ...
        return (long) signCount;
    }

    @Override
    public Map<String, Boolean> getSignInfo(String userKey, LocalDate date) {
        int lengthOfMonth = date.lengthOfMonth();
        Map<String, Boolean> signMap = new TreeMap<>();
        List<Long> list = bitfield(buildSignKey(userKey, date), lengthOfMonth, 0);
        if (!CollectionUtils.isEmpty(list)) {
            long v = list.get(0) == null ? 0 : list.get(0);
            //0表示未签  1表示已签
            for (int i = lengthOfMonth; i > 0; i--) {
                LocalDate localDate = date.withDayOfMonth(i);
                signMap.put(formatDate(localDate, "yyyy-MM-dd"), v >> 1 << 1 != v);
                v >>= 1;
            }
        }
        return signMap;
